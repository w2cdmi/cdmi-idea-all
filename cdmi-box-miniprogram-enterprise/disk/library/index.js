...
Page({
        ...
        if (jumpType != undefined && jumpType != "") {
            if (jumpType == "uploadImage") {
                ...
                if (tempCrumbs != undefined && tempCrumbs.length > 0 && tempFiles != undefined && tempFiles.length > 0) {
                    ...
                    wx.setStorageSync("fileListCrumbs", tempCrumbs);
                } else {
                    wx.showToast({
                        title: '上传失败！',
                        icon: 'none'
            ...
            if (stringUtils.isEmpty(ownerId) && stringUtils.isEmpty(nodeId) && stringUtils.isEmpty(optionsName)) {
                File.listEnterpriseSpace(getApp().globalData.token, getApp().globalData.cloudUserId, 0, 10, function (data) {
                    var team = data.memberships[0].teamspace;
                    ownerId = team.id;
                    nodeId = 0;
                    optionsName = "企业文库";

                    lsOfFolder(getApp().globalData.token, team.id, 0, page, team.name, team.id, 0);
                });
            } else {
    ...
    onShow: function () {
        ...
        //考虑上传文件刷新当前页面，预览文件不刷新当前页面
        if (!isFirstLoad && !page.data.isPreviewFile) {
            page.data.isPreviewFile = false;
