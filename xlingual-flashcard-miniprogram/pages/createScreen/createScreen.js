...
Page({
    ...
    onLoad: function(options) {
        ...
        var createtype = options.type
        var scene = ""
        var destLang =[];
        var editSceneCheckedLang = []
        if (createtype == "leadingScene") {
            scene = wx.getStorageSync("leadingScene")
            destLang = wx.getStorageSync("leadingScene").languages
            page.setData({
                scene: "leadingScene"
            })
        } else if (createtype == "editScene") {
            scene = wx.getStorageSync("editScene")
            destLang = wx.getStorageSync("editScene").languages
            page.setData({
                scene: "editScene"
            })
        }
        var listDetail = scene.cards;
        ...
        for (var i = 0; i < destLang.length; i++) {
            names[destLang[i]] = ""
            audios[destLang[i]] = ""
            editSceneCheckedLang.push({
                "code": destLang[i],
                "checked": true
            })

        }
        wx.setStorageSync("editSceneCheckedLang", editSceneCheckedLang)
        ...
        for (var i = 0; i < listDetail.length; i++) {
            for (var j = 0; j < destLang.length; j++) {
                if (listDetail[i].names[destLang[j]] == undefined) {
                    listDetail[i].names[destLang[j]] = ""
                }
                if (listDetail[i].audios[destLang[j]] == undefined) {
        ...
        page.setData({
            ...
            currentText: scene.cards[0].names[destLang[0]],
            currentAudio: scene.cards[0].audios[destLang[0]],
    ...
    onShow: function() {
        ...
        console.log(listDetail)
        var destLang = wx.getStorageSync(page.data.scene).languages;
        ...
        console.log(newDestLang)
        console.log(destLang)
        for (var i = 0; i < listDetail.length; i++) {
            for (var j = 0; j < destLang.length; j++) {
                if (newDestLang.indexOf(destLang[j].toString()) === -1) {
                    listDetail[i].names[destLang[j]] = ""
                    listDetail[i].audios[destLang[j]] = ""
                }
            }
                ...
                wx.getImageInfo({
                    ...
                    success: function(res) {
                        ...
                        if (str > 1) {
                            ...
                            scaleHeight = height;
                          var w_scale = page.data.width / height;
                          ctx.drawImage(res.path, -ctxY * w_scale, 0, width + 2 * (ctxY * w_scale), height + 2 * ctxY); 
                          ...
                          // console.info("相片宽度:" + width + " 相片高度:" + page.data.height);
                          var w_scale = page.data.height / width;
                          ctx.drawImage(res.path, 0, -ctxX * w_scale, width + 2 * ctxX, page.data.height + 2* (ctxX * w_scale)); 
                          // if (width < page.data.height){
                          //   // ctx.scale(1/w_scale, 1/w_scale);
                          //   // ctx.drawImage(res.path,0,0, page.data.height, page.data.height);
                          //   // ctx.scale(w_scale,w_scale);
                          //   ctx.drawImage(res.path, ctxX, 0, width + ctxX, page.data.height + ctxX * w_scale);
                          // }else{
                          //   ctx.drawImage(res.path, ctxX, 0, width + ctxX, page.data.height + ctxX * w_scale); 
                          // }
                            ctx.draw()
    ...
    setChooseLang: function(e) {
        var page = this
        wx.navigateTo({
            url: '/pages/myAccount/learnLanguage/learnLanguage?chooseType=editSceneLang&scene='+page.data.scene,
        })
    },
    //删除卡片
    deleteCard: function() {
...
var setLeadingScene = ({
    ...
    var categoryName = page.data.categoryName;
    var sceneName = page.data.sceneName;
    var id = wx.getStorageSync("editScene").id
    ...
    if (page.data.scene =="leadingScene"){
        var data = {
    ...
    }else if(page.data.scene =="editScene"){
        var data = {
            id: id,
            author: wx.getStorageSync("loginInfo").userId,
            cards: listDetail,
        ...
        Find.releaseScreen(id, data).then((res) => {
            wx.showToast({
                title: '上传场景中，请稍候',
                icon: "none"
            })
